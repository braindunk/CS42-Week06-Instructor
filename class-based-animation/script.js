...
class Scene1 extends Phaser.Scene {
  ...
  create () {
    ...
    for (let i = 0; i < this.custom.birds; i++) {
      this.custom.gameobjs[i] = this.add.sprite(this.game.scale.gameSize.width, this.game.scale.gameSize.height / this.custom.birds * i, 'seagull1').play('fly');
      this.custom.gameobjs[i].originaly = this.game.scale.gameSize.height / this.custom.birds * i;
  
      this.tweens.add({
        targets: this.custom.gameobjs[i],
        paused: false,
        delay: 0,  
        duration: 1000 - (i * this.custom.birds), 
        ease: 'Linear',
